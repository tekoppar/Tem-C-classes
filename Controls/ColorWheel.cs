...
namespace Tem.TemUI
{
    public partial class ColorWheel : Form {
        ...
        private HSV ActiveColor = new HSV();
        ...
        public ColorWheel(Color? color = null) {
            InitializeComponent();
            this.ActiveColor = HSV.ColorToHSV((color == null ? Color.White: color.Value));
        ...
        private void UpdateActiveColor() {
            Color color = HSV.ColorFromHSV(this.ActiveColor.h, this.ActiveColor.s, this.ActiveColor.b);
        ...
        private void DrawColorRainbow() {
            ...
            for (int x = 0; x < 40; x++) {
                for (int i = 0; i < 360; i++) {
                    hbitmap.SetPixel(x, i, HSV.ColorFromHSV(hsv));
                    hsv.h = i;
                }
            }
        ...
        private void DrawColorBox() {
            ...
            for (int x = 0; x < 256; x++) {
                for (int y = 0; y < 256; y++) {
                    ...
                    hsvColor.b = yV;
                    hsvColor.s = xV;
                    tempColor = HSV.ColorFromHSV(hsvColor);
        ...
        private void ColorWheel_Paint(object sender, PaintEventArgs e) {
            ...
            g.DrawImage(CustomClasses.Properties.Resources.colorpickerbackground, p.X, p.Y, 256, 256);
            g.DrawImage(this.ColorBox, p.X, p.Y, 256, 256);
            g.DrawImage(CustomClasses.Properties.Resources.colorpicker, Math.Max(0, this.ColorpickerLocation.X - 16), Math.Max(0, this.ColorpickerLocation.Y - 16), 16, 16);
